    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xl saturate-150 z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300">
            <div className="bg-[#1d1d1f] p-8 md:p-10 max-w-md w-full relative overflow-hidden rounded-2xl shadow-apple">
                <button onClick={onClose} className="absolute top-6 right-6 text-white/40 hover:text-white transition-colors z-10 p-2 rounded-full hover:bg-white/10">
                ...
                <div className="relative z-10">
                    <div className="flex flex-col mb-8 text-center items-center">
                        ...
                        <span className="text-[14px] font-sans text-white/50 mt-1">创建或加入一个战役</span>
                    </div>
                    ...
                    {commState === 'DISCONNECTED' && (
                        <div className="space-y-6">
                            ...
                            <div className="flex bg-black/30 p-1 rounded-xl">
                                <button onClick={() => { setMode('join'); setConnectionError(null); }} className={`flex-1 py-2 text-[14px] font-sans font-medium rounded-lg transition-all ${mode === 'join' ? 'bg-[#272729] text-white shadow-sm' : 'text-white/50 hover:text-white'}`}>加入房间</button>
                                <button onClick={() => { setMode('create'); setConnectionError(null); }} className={`flex-1 py-2 text-[14px] font-sans font-medium rounded-lg transition-all ${mode === 'create' ? 'bg-[#272729] text-white shadow-sm' : 'text-white/50 hover:text-white'}`}>创建房间</button>
                            </div>

                            <div className="space-y-5">
                                <div className="group">
                                    <label className="block text-[13px] font-sans font-medium text-white/60 mb-2 transition-colors">您的昵称</label>
                                    <input type="text" value={inputName} onChange={e => setInputName(e.target.value)} placeholder="输入您的昵称..." className="w-full bg-black/20 focus:bg-black/40 rounded-xl px-4 py-3 text-white font-sans outline-none focus:ring-2 focus:ring-apple-blue transition-all placeholder:text-white/30 text-[15px]" />
                                </div>
                                {mode === 'join' ? (
                                    <div className="group">
                                        <label className="block text-[13px] font-sans font-medium text-white/60 mb-2 transition-colors">房间 ID</label>
                                        <input type="text" value={inputRoomId} onChange={e => setInputRoomId(e.target.value)} placeholder="输入5位代码" className="w-full bg-black/20 focus:bg-black/40 rounded-xl px-4 py-3 text-white font-sans outline-none focus:ring-2 focus:ring-apple-blue transition-all placeholder:text-white/30 text-[15px] uppercase" />
                                    </div>
                                ) : (
                                    <div className="space-y-5 animate-in fade-in slide-in-from-top-2 duration-300">
                                        <div className="group">
                                            <label className="block text-[13px] font-sans font-medium text-white/60 mb-2 transition-colors">房间名称 (选填)</label>
                                            <input type="text" value={inputRoomName} onChange={e => setInputRoomName(e.target.value)} placeholder="给房间起个名字..." className="w-full bg-black/20 focus:bg-black/40 rounded-xl px-4 py-3 text-white font-sans outline-none focus:ring-2 focus:ring-apple-blue transition-all placeholder:text-white/30 text-[15px]" />
                                        </div>
                                        <div className="group">
                                            <label className="block text-[13px] font-sans font-medium text-white/60 mb-2 transition-colors">选用规则模板</label>
                                ...
                                <div className="bg-black/20 rounded-xl p-5 space-y-4">
                                    ...
                                    {!guestMode && (
                                        <div className="animate-in slide-in-from-top-2 duration-400 mt-4">
                                            <label className="block text-[13px] font-sans font-medium text-white/60 mb-2">关联角色档案</label>
                                            {myCharacters.length === 0 ? (
                                                <div className="text-[14px] text-white/40 font-sans text-center bg-black/20 rounded-lg p-3">
                                                    您的角色库中尚无存档。
                                                </div>
                                            ) : (
                    ...
                    {commState === 'WAITING' && (
                        <div className="flex flex-col items-center justify-center py-10">
                            ...
                            <p className="text-[14px] text-white/50 font-sans mb-8">
                    ...
                    {commState === 'CONNECTED' && (
                        <div className="flex flex-col items-center justify-center py-8">
                            ...
                            <span className="text-[14px] font-sans text-white/50 mb-8 bg-black/20 px-3 py-1 rounded-full">房间 ID: {roomId}</span>
