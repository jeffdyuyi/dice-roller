    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="group">
                    <label className="block text-[10px] font-black text-slate-400 mb-2 uppercase tracking-widest group-within:text-indigo-500 transition-colors">冒险职业 (Class)</label>
                ...
                <div className="group">
                    <label className="block text-[10px] font-black text-slate-400 mb-2 uppercase tracking-widest group-within:text-indigo-500 transition-colors">出生种族 (Race)</label>
            ...
            <div className="space-y-4">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest">六大基础属性 (Abilities)</label>
                <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
                    {['str', 'dex', 'con', 'int', 'wis', 'cha'].map(ab => (
                        <div key={ab} className="flex flex-col gap-2 p-3 bg-white border-2 border-slate-100 hover:border-indigo-200 rounded-2xl transition-all">
                            <span className="text-[10px] font-black text-slate-400 uppercase text-center">{ab}</span>
                            <input type="number" value={data.abilities?.[ab] || 10} onChange={e => updateAbility(ab, parseInt(e.target.value) || 0)} className="w-full bg-slate-50 rounded-xl py-2 text-slate-800 font-black text-center outline-none ring-indigo-500 focus:ring-2" />
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6 pt-4 border-t border-slate-100">
                <div className="group">
                    <label className="block text-[10px] font-black text-slate-400 mb-2 uppercase tracking-widest group-within:text-red-500 transition-colors">最大生命值 (Max HP)</label>
                ...
                <div className="group">
                    <label className="block text-[10px] font-black text-slate-400 mb-2 uppercase tracking-widest group-within:text-orange-500 transition-colors">初始等级 (Level)</label>
